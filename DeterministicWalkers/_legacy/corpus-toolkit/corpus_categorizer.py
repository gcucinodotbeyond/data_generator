...
class DialogueCorpusCategorizer:
    ...
    def __init__(self, model_name: str = 'sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2'):
        ...
        # Pattern per estrazione slot
        time_keywords = r'(?:oggi|domani|dopodomani|adesso|stasera|mattina|pomeriggio|sera|notte|alle|il|tra|fra)'
        self.slot_patterns = {
            'destination': r'\b(per|a|verso)\s+((?:(?!' + time_keywords + r')\w+(?:\s+(?!' + time_keywords + r')\w+)*))',
            'date': r'\b(oggi|domani|dopodomani|lunedì|martedì|mercoledì|giovedì|venerdì|sabato|domenica|\d{1,2}[/-]\d{1,2}(?:[/-]\d{2,4})?)\b',
            'time': r'\b(mattina|pomeriggio|sera|notte|\d{1,2}:\d{2}|alle\s+\d{1,2}(?::\d{2})?)\b',
